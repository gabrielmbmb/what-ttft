...
func TestWriteRunWritesMultiTargetMetadataAndMarkdown(t *testing.T) {
	...
	for _, want := range []string{"## Target comparison", "| target | provider | api | requested tier | observed tier | model | ok | err |", "target-a", "target-b", "responses", "completion_tokens total", "completion_token_records", "e2e_output_tps mean", "generation_delta_output_tps mean", "generation_delta_output_tps count"} {
